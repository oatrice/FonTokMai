...
- **Automated TMD Radar Processing & Nowcasting**: Real-time extraction of rain intensity directly from TMD radar imagery, fully integrated as a highly accurate data source in the automated fallback system. Includes optical flow extrapolation, azimuthal projection, a module-level global cache with locks to prevent concurrency stampedes, and a unified Firebase Storage/Firestore caching system (`radar_latest_cache`) for precise rain cell tracking, forecasting, and high-concurrency rate-limit prevention using efficient static frame comparisons.
