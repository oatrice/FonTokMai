...
- Step 2: Open another terminal and simulate a Telegram location message using `curl`:
  `curl -X POST http://localhost:8001/api/v1/webhook/telegram -H "Content-Type: application/json" -d '{"message": {"chat": {"id": 12345}, "location": {"latitude": 13.75, "longitude": 100.50}}}'`
...
- Step 3: Simulate the user clicking the "จำ 2 เดือน" (loc_2m) inline button by sending a `callback_query` payload:
  `curl -X POST http://localhost:8001/api/v1/webhook/telegram -H "Content-Type: application/json" -d '{"callback_query": {"id": "query_1", "from": {"id": 12345}, "message": {"message_id": 1, "chat": {"id": 12345}}, "data": "loc_2m_13.75_100.50"}}'`
...
- Step 4: Simulate the user sending the `/mylocation` command:
  `curl -X POST http://localhost:8001/api/v1/webhook/telegram -H "Content-Type: application/json" -d '{"message": {"chat": {"id": 12345}, "text": "/mylocation"}}'`
...
- Step 5: Simulate the user clicking the "🗑️ ลบพิกัดเดิม" button to delete the location:
  `curl -X POST http://localhost:8001/api/v1/webhook/telegram -H "Content-Type: application/json" -d '{"callback_query": {"id": "query_2", "from": {"id": 12345}, "message": {"message_id": 2, "chat": {"id": 12345}}, "data": "loc_del"}}'`
