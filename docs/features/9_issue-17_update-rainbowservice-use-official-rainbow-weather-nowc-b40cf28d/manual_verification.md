...
- Step 6: ทดสอบฟีเจอร์ปุ่มสลับ Endpoint (Endpoint Switcher) แบบ End-to-End
  1. หลังจากบอทตอบกลับข้อมูลพยากรณ์พร้อมปุ่มจาก Step 5
  2. ให้กดปุ่ม `🔄 สลับไปใช้ Local Radar` ในหน้าแชท
  3. **ข้อความเดิมต้องถูกแก้ (Edit)** โดยไม่เด้งเป็นข้อความแชทใหม่
  4. เนื้อหาข้อความต้องระบุว่า `(ตรวจสอบด้วย: Rainbow Local Radar)`
  5. กดปุ่ม `🔄 สลับไปใช้ Global` อีกครั้ง ข้อความต้องแก้กลับเป็น `(ตรวจสอบด้วย: Rainbow Global)` และทำงานได้ลื่นไหล
