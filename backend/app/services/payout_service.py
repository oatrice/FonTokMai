...
Uses async SQLAlchemy session to match the project's async-first architecture.
"""
import logging
import datetime
...
class PayoutService:
    ...
    async def _find_payout(self, payout_id: str) -> Payout | None:
    ...
    async def handle_payout_created(self, payout_data: Dict[str, Any]) -> Dict:
        ...
        existing = await self._find_payout(payout_id)
        if existing:
    ...
    async def handle_payout_paid(self, payout_data: Dict[str, Any]) -> Dict:
        ...
        """
        payout_id = payout_data["id"]
        payout = await self._find_payout(payout_id)
    ...
    async def handle_payout_failed(self, payout_data: Dict[str, Any]) -> Dict:
