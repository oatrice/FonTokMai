...
echo "=================================================="
echo "🔓 Restoring Public Access to $SERVICE_NAME"
echo "=================================================="
...
echo "✅ Public access restored successfully."
...
if [ -n "$TELEGRAM_BOT_TOKEN" ] && [ -n "$DEVELOPER_CHAT_IDS" ]; then
  ...
  for CHAT_ID in $CHAT_IDS; do
    curl -s -X POST "https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage" \
      ...
      -d "{
        \"chat_id\": \"${CHAT_ID}\",
        \"text\": \"✅ <b>Service Restored</b>\n\nระบบ <code>${SERVICE_NAME}</code> ถูกกู้คืนสิทธิ์ Public Access เรียบร้อยแล้ว (พร้อมรับ Traffic ตามปกติ)\",
        \"parse_mode\": \"HTML\"
      }" > /dev/null
  done
